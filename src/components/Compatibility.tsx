      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          <span className="inline-block text-sm font-semibold px-4 py-1.5 rounded-full mb-4" style={{ background: 'rgba(45,111,165,0.1)', color: '#2d6fa5' }}>
            {tr(lang, '🧭 Marriage Compatibility')}
          </span>
          <h2 className="text-3xl sm:text-4xl font-bold mb-4" style={{ color: '#1a1a2e' }}>
            {tr(lang, 'Compatibility Measured on What Marriage Is Built On')}
          </h2>
          <p className="text-gray-500 max-w-2xl mx-auto text-lg">
            {tr(lang, 'Not photos and small talk — deen, family, children, finances, and timeline.')}{' '}
            {tr(lang, 'Every score is explained, so you understand')} <em>{tr(lang, 'why')}</em> {tr(lang, 'a candidate is compatible.')}
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          {/* Left: dimensions */}
          <div>
            <h3 className="font-bold text-gray-800 mb-4">{tr(lang, 'Assessed across 15 marriage dimensions')}</h3>
            ...
            <p className="text-gray-500 text-sm mt-6 leading-relaxed">
              {tr(lang, 'You complete the assessment once during onboarding. We then introduce you only to verified candidates whose answers align with yours — and show you exactly where you align and where you differ, so families can make informed decisions together.')}
            </p>
          </div>
          ...
          <div className="rounded-3xl p-6 shadow-xl border border-gray-100 bg-white">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-gray-800 text-sm">{tr(lang, 'Compatibility Report')}</span>
                <span className="text-xs text-gray-400">{tr(lang, '(example)')}</span>
              </div>
              ...
              {EXAMPLE_FACTORS.map(f => (
                <div key={f.label}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-semibold text-gray-700">{tr(lang, f.label)}</span>
                  ...
                  <p className="text-xs text-gray-400 mt-1">{tr(lang, f.note)}</p>
                </div>
              ))}
            </div>
          </div>
