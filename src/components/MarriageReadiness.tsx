      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          <span className="inline-block text-sm font-semibold px-4 py-1.5 rounded-full mb-4" style={{ background: 'rgba(26,107,74,0.1)', color: '#1a6b4a' }}>
            {tr(lang, '🎖️ Marriage Readiness')}
          </span>
          <h2 className="text-3xl sm:text-4xl font-bold mb-4" style={{ color: '#1a1a2e' }}>
            {tr(lang, 'Know Who Is Actually Ready for Nikah')}
          </h2>
          <p className="text-gray-500 max-w-2xl mx-auto text-lg">
            {tr(lang, 'Every member earns a Marriage Readiness badge based on verified behavior — not self-promotion. Filter your candidates by readiness so no one wastes your time.')}
          </p>
        </div>
          ...
          {BADGES.map(b => {
            ...
            return (
              <div key={b.label} className="p-5 rounded-2xl bg-white border border-gray-100 text-center hover:shadow-lg transition-all">
                ...
                <p className="font-bold text-sm mb-1" style={{ color: b.color }}>{tr(lang, b.label)}</p>
                <p className="text-gray-400 text-xs leading-relaxed">{tr(lang, b.desc)}</p>
              </div>
            )
          })}
        </div>
        ...
        <div className="rounded-2xl p-6 bg-white border border-gray-100 text-center">
          <p className="font-semibold text-gray-700 text-sm mb-4">{tr(lang, 'Your readiness score is built from:')}</p>
